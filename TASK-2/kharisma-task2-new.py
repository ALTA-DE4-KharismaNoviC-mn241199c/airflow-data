...
create_table_task = PostgresOperator(
    task_id='create_table_to_postgres_kharisma',
    sql=(
        'CREATE TABLE IF NOT EXISTS kharisma_gender_name_prediction_task2 ('
        'input JSONB,'
        'details JSONB,'
    ...
    ')'),
    postgres_conn_id='pg_conn_kharisma',
    retries=3,
...
def load_predictions_to_postgres(**kwargs):
    ti = kwargs['ti']
    predictions = ti.xcom_pull(task_ids='profile_from_gender_kharisma')
    ...
    for prediction in predictions:
        ...
        pg_hook.run("""
            INSERT INTO kharisma_gender_name_prediction_task2 (input, details, result_found, first_name, probability, gender)
            VALUES (%s, %s, %s, %s, %s, %s);
